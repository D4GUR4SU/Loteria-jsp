...
public abstract class DAO<T extends Model> {
	...
	/**
	 * Deleta um Objeto passado como parâmetro no banco de dados OBS: Este Objeto
	 * deve ter o ID informado
	 * 
	 * @param object
	 */
	public void delete(T model) {
		if (model != null && model.getId() < 1) {
			throw new RuntimeException("Não foi Possível deletar pois o id é 0 ou inválido!  classe do registro:"
					+ model.getClass().getSimpleName() + " id do registro: " + model.getId());
		}
	...
	public T selectPorId(T model) {
		...
		if (model == null) {
			...
			throw new RuntimeException("O Registro que tentou buscar, não existe no Banco de Dados! classe do registro:"
					+ persistedClass.getSimpleName() + " id do registro: " + id);
		}

		return model; // Sera retornado o registro encontrado, ou nulo, ou se tiver inativo estoura a
						// exeption
	...
	public T selectPorIdAtivo(T model) throws Exception {
		...
		if (model != null) {
			if (!model.isAtivo()) {
				throw new Exception("O Registro que tentou buscar, esta inativo no Banco de Dados! classe do registro:"
						+ model.getClass().getSimpleName() + "O id do registro: " + model.getId());
			}
		}
